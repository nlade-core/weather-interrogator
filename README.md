...
- Precip wash is coloured by type (blue rain/drizzle, pale cyan snow, violet thunderstorm/hail) from `weathercode` — confirmed against the docs that weathercode is an *instant* reading (unlike probability, no hour-shift needed), so it's a plain "most recent known reading" lookup. Small condition icons sit at each hour mark along the axis
...
- [ ] Precipitation severity/intensity (mm, from `precipitation`/`rain`/`showers`/`snowfall`) — type is in (via `weathercode`), intensity still deferred
