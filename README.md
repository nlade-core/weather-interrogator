...
- "Ask" is wired to Chrome's built-in Gemini Nano (Prompt API) — on-device, no server. Edinburgh's 15-min temperature, 15-min rain amount (mm), and hourly conditions (type) for the rest of today are all baked into one persistent session's system prompt at creation time, so follow-up questions share context instead of starting fresh each time. No rain-probability figure is included — deliberately, since the only one available wasn't locally reliable
...
- [ ] Broaden the fetched fields into the model context and chart: confirmed the *full* UKV 2km variable list (see Data notes) includes `apparent_temperature`, `uv_index`, `wind_gusts_10m`/`wind_direction_10m`, `cloud_cover`, `cape`, `freezing_level_height` — all genuine 15-min resolution, same coherent source (now explicitly pinned) as everything else already shown. Needed before Ask can answer judgment-call questions ("is it worth a BBQ") rather than temperature/rain-only ones
